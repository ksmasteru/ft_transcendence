...
import { JWT_SECRET, JWT_EXPIRATION, UID, SECRET,  CALLBACK_URL} from '../conf/env.js';
import {sendVerificationEmail , sendResetPasswordEmail } from '../conf/nodemailer.js';
    ...
    try {
        ...
        if (typeof request.body === 'string') {
            try {
                ...
                return reply.status(400).send({ error: 'Invalid JSON format in request body.' });
            }
        } else {
            body = request.body ?? {};
        }
        ...
        try {
            await sendVerificationEmail({ _id: newUser.id, email: newUser.email }, reply);
            
            return reply.status(201).send({
                message: 'User registered successfully! Please check your email and verify your account before signing in.',
                data: {
                    id: newUser.id,
        ...
        return reply.status(500).send({ error: 'An error occurred during sign up.' });
    }
};
...
export const callback42 = async (request, reply) => {
  ...
  try {
    ...
    user = await prisma.user.create({
        ...
        name: intraUser.usual_full_name || `${intraUser.first_name} ${intraUser.last_name}`,
        password: "null",
        verified: true,
        avatar: profileImage, 
        },
...
export const verifyEmail = async (request, reply) => {
    ...
    try {
        if (!userId || !uniqueString) {
            return reply.status(400).send({ error: 'User ID and unique string are required.' });
        }
        ...
        if (!userVerification) {
            return reply.status(404).send({ 
                error: 'Verification record not found. Please request a new verification email.' 
            });
        }
        ...
        return reply.status(500).send({ error: 'An error occurred during email verification.' });
    }
};
...
export const resendVerification = async (request, reply) => {
    try {
        ...
        return reply.status(500).send({ error: 'An error occurred while resending verification email.' });
    }
};
...
export const signOut = async (request, reply) => {
  try {
    ...
    return reply.status(500).send({ error: 'An error occurred during sign out.' });
  }
};
...
export const forgotPassword = async (request, reply) => {
    try {
        ...
        await sendResetPasswordEmail({ email: user.email, resetToken, _id: user.id });
...
export const setup2FA = async (request, reply) => {
    try {
        ...
        if (!user.email) {
            ...
            return reply.status(400).send({ error: 'User account is missing an email, cannot set up 2FA.' });
        }
        ...
        return reply.status(200).send({
            message: '2FA setup initiated successfully. Please scan the QR code and verify.',
        ...
        return reply.status(500).send({ error: 'An error occurred while setting up 2FA.' });
    }
};
...
export const verify2FA = async (request, reply) => {
    try {
        ...
        if (!/^\d{6}$/.test(token)) {
            return reply.status(400).send({ error: 'Token must be a 6-digit number.' });
        }
        ...
        if (!user.twoFactorSecret) {
            return reply.status(400).send({ error: '2FA setup not initiated. Please start setup first.' });
        }
        ...
        if (!verified) {
            return reply.status(400).send({ error: 'Invalid token. Please try again.' });
        }
        ...
        return reply.status(500).send({ error: 'An error occurred while verifying 2FA.' });
    }
};
...
export const disable2FA = async (request, reply) => {
    try {
        ...
        if (token) {
            ...
            if (!verified) {
                return reply.status(400).send({ error: 'Invalid token. Cannot disable 2FA.' });
            }
        }
        ...
        return reply.status(500).send({ error: 'An error occurred while disabling 2FA.' });
    }
};
...
export const verifyLogin2FA = async (request, reply) => {
    try {
        ...
        return reply.status(500).send({ error: 'An error occurred while verifying 2FA token.' });
    }
};
...
export const get2FAStatus = async (request, reply) => {
    try {
        ...
        return reply.status(500).send({ error: 'An error occurred while getting 2FA status.' });
    }
};
...
export const reset2FA =  async (request, reply) => {
    ...
    try {
        ...
        return reply.status(500).send({ error: 'An error occurred while resetting 2FA.' });
    }
};
...
export const changePassword =  async (request, reply) => {
    ...
    if (!userId || !oldPassword || !newPassword) {
        return reply.status(400).send({ error: 'User ID, old password, and new password are required.' });
    }
    ...
    try {
        ...
        const isPasswordValid = await fastify.bcrypt.compare(oldPassword, user.password);
        if (!isPasswordValid) {
        ...
        return reply.status(500).send({ error: 'An error occurred while changing the password.' });
    }
};
...
export const resetPassword = async (request, reply) => {
    try {
        ...
        return reply.status(500).send({ error: 'An error occurred while resetting the password.' })
    }
};